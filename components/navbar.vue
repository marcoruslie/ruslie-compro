	...
	<nav class="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-lg shadow-md transition-all duration-300">
		<div class="max-w-7xl mx-auto flex justify-between items-center px-6 md:px-10 py-3">
			...
			<div class="flex items-center gap-3">
				<img src="/Logo_Ruslie_Spring.png" alt="Ruslie Spring Logo"
					class="w-12 h-12 md:w-14 md:h-14 object-contain rounded-full border border-blue-100 shadow-sm" />
				<div>
					<h1 class="text-xl md:text-2xl font-extrabold text-[#033272] tracking-tight">
						Ruslie Spring
					</h1>
			...
			<!-- Mobile Menu Icon -->
			<button class="md:hidden text-gray-700 hover:text-blue-700 transition" @click="mobileMenu = !mobileMenu">
				<svg v-if="!mobileMenu" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
		...
		<transition name="fade">
			<div v-if="mobileMenu" class="md:hidden bg-white/95 backdrop-blur-lg border-t shadow-inner py-4">
				<ul class="flex flex-col items-center gap-4 text-gray-700 font-medium">
					...
					<li>
						<NuxtLink to="/calculator" class="hover:text-blue-700 transition" @click="mobileMenu = false">
							Calculator</NuxtLink>
					</li>
				</ul>
			</div>
